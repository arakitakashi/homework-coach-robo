...
### v1.1 (2026-01-29)
- **フロントエンドランタイムをBunに変更**（セクション1.1）
  - Node.js 20+ → Bun 1.0+
  - Bun採用理由を追加
  - Dockerイメージをoven/bunに変更
- **状態管理をJotaiに変更**（セクション1.1）
  - Zustand → Jotai（Atomic状態管理）
  - Jotai採用理由と実装例を追加
- **バックエンドパッケージマネージャをuvに決定**（セクション2.1）
  - pip/uvの選択肢 → uv決定
  - uvインストール手順と利点を追加
  - CI/CDをuvに対応
- **Dockerfileを最適化**
  - フロントエンド: Bunベースイメージ（イメージサイズ削減）
  - バックエンド: uv統合（ビルド時間短縮）
