...
### 1.1 Firestoreに保存するデータの定義

**アプリケーションデータ** = ユーザー情報、問題バンク、カリキュラムなど、アプリケーション固有のマスターデータおよびユーザーデータ

> **注**: セッション状態とメモリ（対話履歴のコンテキスト）は Vertex AI Agent Engine（SessionService / MemoryBankService）で管理されています。Firestoreはアプリケーションデータの永続化に使用します。
...
- **Vertex AI Agent Engine**: セッション管理・メモリ管理（SessionService / MemoryBankService）
- **Firestore**: アプリケーションデータ（ユーザー情報、問題バンク、カリキュラム、学習プロファイル）
