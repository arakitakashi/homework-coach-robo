...
3. **品質チェック**: `/quality-check` スキルでサブエージェントに委譲（mypy/ruff/pytest の一括実行）
4. **ドキュメント更新**: `/update-docs` スキルでサブエージェントに委譲（CLAUDE.md, docs/ の更新）

**サブエージェント委譲のポイント**:
- 品質チェックの出力（数百〜数千行）やドキュメント読み込み（700行以上）がメインのコンテキストに載ることを防ぐ
- サブエージェントが作業を完了し、結果サマリーのみがメインに返る
